...
### 1. Swap the avatar for a real model (5–15 min)
- Download a free anime-style **VRM** from [VRoid Hub](https://hub.vroid.com) or a rigged **GLB** from Sketchfab (filter: Downloadable + Creative Commons).
- Good starting models (CC-BY):
  - “Annie Anime Girl” by Tatenashi
  - “Rigged Anime Japanese High School Girl”
  - Any free VRoid export
- Replace the procedural `avatar` group with:
...
loader.load('your-model.vrm', (gltf) => {
  const vrm = gltf.userData.vrm;
  // position, scale, then scene.add(vrm.scene)
  // use vrm.expressionManager for smiles, blinks, etc.
});
```

`@pixiv/three-vrm` gives you blendshapes, look-at, spring bones — perfect for anime companions.
...
### 4. Add more polish
- Web Speech API or ElevenLabs for voice + simple lip-sync (jaw morph or visemes)
...
## Legal / Design note
This is an **original** character and design. Do not copy Ani’s exact appearance, name, or copyrighted assets. Create your own look (VRoid makes this trivial).
...
Want a full Vue + TresJS + VRM scaffold next? Just say the word and we’ll push it.
